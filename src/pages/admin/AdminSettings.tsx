    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="branding">
          <BrandingSettings settings={settings} onUpdate={updateSettings.mutateAsync} isUpdating={updateSettings.isPending} />
        </TabsContent>

        <TabsContent value="colors">
          <ColorSettings settings={settings} onUpdate={updateSettings.mutateAsync} isUpdating={updateSettings.isPending} />
        </TabsContent>

        <TabsContent value="contact">
          <ContactSettings settings={settings} onUpdate={updateSettings.mutateAsync} isUpdating={updateSettings.isPending} />
        </TabsContent>

        <TabsContent value="hero">
          <HeroSettings settings={settings} onUpdate={updateSettings.mutateAsync} isUpdating={updateSettings.isPending} />
        </TabsContent>

        <TabsContent value="menu">
          <MenuSettings settings={settings} onUpdate={updateSettings.mutateAsync} isUpdating={updateSettings.isPending} />
        </TabsContent>
...
interface SettingsCardProps {
  ...
  onUpdate: (data: Partial<SiteSettings>) => Promise<SiteSettings>;
  isUpdating: boolean;
}
...
const BrandingSettings = ({ settings, onUpdate, isUpdating }: SettingsCardProps) => {
  ...
  const handleSave = async () => {
    try {
      await onUpdate({ brand_name: brandName, tagline, logo_url: logoUrl || null });
...
const HeroSettings = ({ settings, onUpdate, isUpdating }: SettingsCardProps) => {
  ...
  useEffect(() => {
    ...
    setHeroCta(settings.hero_cta_text || '');
    setHeroLink(settings.hero_cta_link || '');
...
const ContentSettings = ({ settings, onUpdate, isUpdating }: SettingsCardProps) => {
  ...
  useEffect(() => {
    ...
    setSafetyContent(settings.safety_content || '');
    setCategoriesTitle(settings.categories_title || '');
    setCategoriesSubtitle(settings.categories_subtitle || '');
    setHowItWorksTitle(settings.how_it_works_title || '');
    setSteps(settings.how_it_works_steps);
  }, [settings]);
